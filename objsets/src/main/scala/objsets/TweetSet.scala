...
abstract class TweetSet extends TweetSetInterface:
  ...
  def descendingByRetweet: TweetList = {
    try
      ...
      return Cons(tweet, remove(tweet).descendingByRetweet)
    catch
      case e: java.util.NoSuchElementException => return Nil
  }
  ...
  override def mostRetweeted: Tweet = {
    ...
    tweet = left match {
      case _: NonEmpty => {
        ...
        if leftTweet.retweets > tweet.retweets then leftTweet else tweet
      }
      case _ => tweet
    }
    tweet = right match {
      case _: NonEmpty => {
        ...
        if rightTweet.retweets > tweet.retweets then rightTweet else tweet
      }
      case _ => tweet
    }
    tweet
