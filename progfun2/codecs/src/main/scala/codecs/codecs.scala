...
trait DecoderInstances:
  ...
  given Decoder[Int] = Decoder.fromFunction {
    case Json.Num(x) => if x.isValidInt then Some(x.toInt) else None
    case _ => None
  }
  ...
  given Decoder[Boolean] = Decoder.fromPartialFunction { case Json.Bool(x) => x }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(as) => {
        val xs = as.map(a => decoder.decode(a))
        if xs.forall(_.isDefined) then Some(xs.map(x => x.get))
        else None
      }
      case _ => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(x) => {
        if x.contains(name) then decoder.decode(x(name))
        else None
      }
      case _ => None
    }
