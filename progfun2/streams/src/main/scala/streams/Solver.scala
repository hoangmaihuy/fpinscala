...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    LazyList.from(b.legalNeighbors.map(p => (p._1, p._2 :: history)))
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
    val allNeighbors = initial.flatMap(p => neighborsWithHistory(p._1, p._2))
    val newNeighbors = newNeighborsOnly(allNeighbors, explored)
    if newNeighbors.isEmpty then
      LazyList()
    else
      initial ++ from(newNeighbors, explored ++ initial.map(_._1))
  }
