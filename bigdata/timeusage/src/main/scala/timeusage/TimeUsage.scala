...
//    finalDfSql.show()
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def containsPrefix(s: String, prefixes: List[String]): Boolean = {
      prefixes.exists(prefix => s.startsWith(prefix))
    }

    def isPrimaryNeedCol(colName: String) = {
      containsPrefix(colName, primaryNeedsPrefixes)
    }

    def isWorkCol(colName: String) = {
      containsPrefix(colName, workPrefixes)
    }

    def isOtherCol(colName: String) = {
      containsPrefix(colName, otherPrefixes) && !isPrimaryNeedCol(colName) && !isWorkCol(colName)
    }

    val primaryNeedsCols = columnNames.filter(isPrimaryNeedCol).map(col)
    val workCols = columnNames.filter(isWorkCol).map(col)
    val otherCols = columnNames.filter(isOtherCol).map(col)
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy(col("working"), col("sex"), col("age")).agg(
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    def round(d: Double, i: Int) = Math.round(d * Math.pow(10, i)) / Math.pow(10, i)
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age)).agg(
      ...
      TimeUsageRow(row._1._1, row._1._2, row._1._3, round(row._2, 1), round(row._3, 1), round(row._4, 1))
    }.sort(col("working"), col("sex"), col("age"))
  }
}
