...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf(loadDefaults = true).setAppName("bigdata-wikipedia").setMaster("local[4]")
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.map(idx => (idx._1, idx._2.size)).collect().sortWith(_._2 > _._2).toList
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap {
      article =>
        langs.map {
          lang => (lang, if (article.mentionsLanguage(lang)) 1 else 0)
        }
    }
    .reduceByKey(_ + _)
    .collect()
